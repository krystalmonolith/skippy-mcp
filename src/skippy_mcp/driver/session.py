...
def establish(transport: Transport, *, reset_on_connect: bool = True) -> Scope:
    ...
    is known. Unknown models are refused by :func:`select_dialect`.
    """
    idn = IdnInfo.parse(transport.query("*IDN?"))
    dialect = select_dialect(idn)
    scope = Scope(transport, dialect, idn)
    if reset_on_connect:
        scope.reset()
    return scope
